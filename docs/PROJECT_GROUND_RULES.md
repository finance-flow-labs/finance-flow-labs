# finance-flow-labs Project Ground Rules (v1.0)
...
## 1) Product Scope (고정)
1. **미국 + 한국 + 암호화폐** 통합 투자 분석
2. 거시경제 분석 (US/KR 중심)
3. 섹터 분석
4. 개별주 분석
5. 한국 자산 범위는 **지수 + ETF + 개별주 모두 포함**
6. 모의투자(포트폴리오/시그널 실행) 및 성과 추적
7. 예상수익률 vs 실현수익률 차이(오차) 분석 및 원인 분류
8. 웹 시각화 대시보드
9. OpenClaw 스킬 기반 온디맨드 리서치/리포트
...
## 4) Risk, Benchmark & Portfolio Control (정책 v1)
> 상세 확정값은 `docs/POLICY_LOCK_V1.md`를 따른다.

- 목표 수익률: **연 20%** (장기 평균 기준)
- 최대 허용 낙폭(MDD) 경보선: **-30%**
- 단일 아이디어 최대 비중: **20%**
- 레버리지 ETF(TQQQ/SOXL/TMF 등) 합산 상한: **20%**
- 벤치마크(복합): **45% QQQ + 25% KOSPI200 proxy + 20% BTC + 10% SGOV**
- 기본 평가 호라이즌: **1M (Primary)**, 보조로 1W/3M 트랙 병행
- 기본 리밸런싱 주기: **분기(Quarterly)**
- 암호화폐 슬리브: BTC/ETH 합계 최소 70%, 상위 알트 합계 최대 30%
...
## 6) Delivery Rules
- 사용자에게는 핵심을 유지하되 **기본 포맷은 상세형**으로 보고
- 기본 보고 주기: **하루 1회 요약 + 중요 이벤트 즉시 알림**
